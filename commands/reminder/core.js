...
exports.createReminder = async ({
  client, userId, channelId, when, text,
  isPrivate, targetUserId, mentionEveryone, zone, reply
}) => {
  ...
  const ackPrefix = whoAck === 'you'
    ? `Okay <@${userId}>. I'll remind you "${text}"`
    : `Okay <@${userId}>. I'll remind ${whoAck} "${text}"`

  if (parsed.mode === 'duration') {
    const remindAt = DateTime.utc().plus({ milliseconds: parsed.delay }).toJSDate()
    await reply(`${ackPrefix} in ${ms(parsed.delay, { long: true })}.`)
    const r = await Reminder.create({
      userId, channelId, text, isPrivate,
      targetUserId: targetUserId || null,
      mentionEveryone: !!mentionEveryone,
      remindAt,
      completed: false, canceled: false, createdAt: new Date()
    })
  ...
  await reply(`${ackPrefix} at ${display}.`)
