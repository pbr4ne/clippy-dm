...
const SENIOR_COLOUR_KEYS = new Set(['grey', 'white', 'salt & pepper', 'silver']);
...
function pickHairColourForAge(age) {
  ...
  if (age < 40) {
    return pick(nonSenior);
  } else if (age < 55) {
    return Math.random() < 0.15 ? pick(senior) : pick(nonSenior);
  } else {
...
function formatHair(hair) {
  if (!hair) return '';
  if (hair.length && hair.length.toLowerCase() === 'shaved') {
    return 'shaved';
  }
